 ...
 * jump under the cursor. Ordering is a property of the minute, not the second.
 */
const REORDER_MS = 20_000;

/**
 * A number from the server, or a dash. A server one version behind should cost
 * the figure it did not send, not the page.
 */
          ...
          <DropTable
            ...
            onSort={(key) => {
              if (sort === key) setDesc(!desc);
                ...
                setSort(key);
                // The interesting end differs per column: the biggest supply
                // and the largest following, but the cheapest floor.
              ...
              <>
                ...
                <WatchButton
                  draft={{
                    ...
                    contract: d.contract,
                    twitter: info[d.contract.toLowerCase()]?.twitter,
                    supply: d.maxSupply,
